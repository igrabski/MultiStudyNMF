...
rd_loglik <- function(s,M_s,W_s,Pr,Er_s,Ar,P,E_s,A,fac) {
...
rd_sample_A <- function(n,A,M,W,P,E,p,S,N,fac,Ar,Pr,Er,gamma) {
  for (s in 1:S) {
    if (gamma==(-1)) {
      Ar1 <- A
      Ar0 <- A
      Ar1[s,n] <- 1
      Ar0[s,n] <- 0
      p1 <- rd_loglik(s,M[[s]],W[[s]],Pr,Er[[s]],Ar,P,E[[s]],Ar1,fac)+log(p[s,n])
      p0 <- rd_loglik(s,M[[s]],W[[s]],Pr,Er[[s]],Ar,P,E[[s]],Ar0,fac)+log(1-p[s,n])
      ...
      p0 <- rd_loglik(s,M[[s]],W[[s]],Pr,Er[[s]],Ar,P,E[[s]],Ar0,fac)
      p1 <- rd_loglik(s,M[[s]],W[[s]],Pr,Er[[s]],Ar,P,E[[s]],Ar1,fac)
      ratio <- (exp((p0-p1)))
      if (runif(1)<ratio) {
        A[s,n] <- prop
      }
...
rd_sample_Ar <- function(n,Ar,M,W,Pr,Er,P,E,A,pr,S,fac,gamma) {
  for (s in 1:S) {
    if (gamma==(-1)) {
      Ar1 <- Ar
      Ar0 <- Ar
      Ar1[s,n] <- 1
      Ar0[s,n] <- 0
      p1 <- rd_loglik(s,M[[s]],W[[s]],Pr,Er[[s]],Ar1,P,E[[s]],A,fac)+log(pr[s,n])
      p0 <- rd_loglik(s,M[[s]],W[[s]],Pr,Er[[s]],Ar0,P,E[[s]],A,fac)+log(1-pr[s,n])
      prob <- exp(p1-sumLog(c(p1,p0)))
      ...
      # Compute acceptance ratio
      p0 <- rd_loglik(s,M[[s]],W[[s]],Pr,Er[[s]],Ar0,P,E[[s]],A,fac)
      p1 <- rd_loglik(s,M[[s]],W[[s]],Pr,Er[[s]],Ar1,P,E[[s]],A,fac)
      ratio <- (exp((p0-p1)))
      if (runif(1)<ratio) {
        Ar[s,n] <- prop
      }
...
## Sample latent count
rd_sample_zijs <- function(i,j,s,M_s,Pstar,Estar,Astar) {
...
rd_sample_ens <- function(n,s,W_s,P,A,Z_s,alpha_e,beta_e,gamma) {
...
rd_alphainp_density <- function(i,n,P,y,beta_p,lambda_p,gamma) {
  return(log(lambda_p)-lambda_p*y+gamma*((y+1)*log(beta_p[i,n])+y*log(P[i,n])-lgamma(y+1)))
}

## Gamma proposal density
rd_gamma_density <- function(y,x) {
  return(dgamma(y,x,1,log=TRUE))
}

## Sample signature hyperparameter alpha
rd_sample_alphainp <- function(i,n,P,alpha_p,beta_p,lambda_p,gamma) {
  x <- alpha_p[i,n]
  y <- rgamma(1,x,1)
  alpha <- rd_alphainp_density(i,n,P,y,beta_p,lambda_p,gamma)-
    rd_alphainp_density(i,n,P,x,beta_p,lambda_p,gamma)+
    rd_gamma_density(x,y)-rd_gamma_density(y,x)
...
rd_alphanjse_density <- function(n,s,E_s,y,beta_e,lambda_e,gamma) {
  return(log(lambda_e)-lambda_e*y+sum(gamma*((y+1)*log(beta_e[[s]][n,]))+
                                        (y*log(E_s[n,]))-lgamma(y+1)))
}

## Sample exposure hyperparameter alpha
rd_sample_alphanse <- function(n,s,E_s,alpha_e,beta_e,lambda_e,gamma) {
  x <- alpha_e[[s]][n,1]
  y <- rgamma(1,x,1)
  alpha <- rd_alphanjse_density(n,s,E_s,y,beta_e,lambda_e,gamma)-
    rd_alphanjse_density(n,s,E_s,x,beta_e,lambda_e,gamma)+
          rd_gamma_density(x,y)-rd_gamma_density(y,x)
  if (runif(1) <= min(1,exp(alpha))) {
    return(y)
  } else {
...
### Full Sampler
recovery_discovery <- function(M,lambda_p=0.5,a_p=5,b_p=0.05,lambda_e=0.1,a_e=5,b_e=0.01,
                               a_er=3,b_er=0.01,lambda_er=0.5,inits=NULL,fixed=FALSE,
                               A.fixed=NULL,first=T) {
  # Identify values
  K <- dim(M[[1]])[1]
  S <- length(M)
  ...
  beta_er <- lapply(1:S,function(s) array(rgamma(Nr*G[s],a_er[s],b_er[s]),dim=c(Nr,G[s])))
  ...
  Er <- lapply(1:S,function(s) array(rgamma(Nr*G[s],alpha_er[[s]]+1,beta_er[[s]]),dim=c(Nr,G[s])))
  Er <- lapply(1:S,function(s) Er[[s]]+min(Er[[s]][Er[[s]]!=0]))
  if (!is.null(inits)) {
    P <- inits[[1]]
    E <- inits[[2]]
    Pr <- inits[[3]]
  ...
  for (s in 1:S) {
    ...
    for (i in 1:K) {
      for (j in 1:G[s]) {
        z <- rd_sample_zijs(i,j,s,M[[s]],Pstar,Estar,Astar)
        for (n in 1:N) {
          Z[[s]][[n]][i,j] <- z[n]
        }
  ...
  for (l in 1:length(params)) {
    ...
    if (params[l]==(-1)) {
      ...
      for (n1 in sigs.order) {
        Pr[,n1] <- rd_sample_pn(n1,W,Er,Ar,Zr,alpha_pr-1,beta_pr,S,G,gamma)
      }
    } else {
      for (n1 in sample(1:Nr,Nr,replace=FALSE)) {
        ...
        while (curr > 0.9 & maxit < 10) {
          Pr[,n1] <- rd_sample_pn(n1,W,Er,Ar,Zr,alpha_pr-1,beta_pr,S,G,temps[l])
    ...
    for (s in 1:S) {
      for (n1 in sigs.order) {
        Er[[s]][n1,] <- rd_sample_ens(n1,s,W[[s]],Pr,Ar,Zr[[s]],alpha_er,beta_er,gamma)
      }
    }
    ...
    if (params[l]==(-1)) {
      ...
      for (n1 in sigs.order) {
        P[,n1] <- rd_sample_pn(n1,W,E,A,Z,alpha_p,beta_p,S,G,gamma)
      }
    } else {
      for (n1 in sample(1:N,N,replace=FALSE)) {
        ...
        curr <- Inf
        while (curr > 0.9 & maxit < 10) {
          P[,n1] <- rd_sample_pn(n1,W,E,A,Z,alpha_p,beta_p,S,G,temps[l])
    ...
    for (s in 1:S) {
      for (n1 in sample(1:N,N,replace=FALSE)) {
        E[[s]][n1,] <- rd_sample_ens(n1,s,W[[s]],P,A,Z[[s]],alpha_e,beta_e,gamma)
      }
    }
    ...
    if (!fixed) {
      sigs.order <- sample(1:Nr,Nr,replace=FALSE)
      for (n in sigs.order) {
        Ar <- rd_sample_Ar(n,Ar,M,W,Pr,Er,P,E,A,pr,S,fac,params[l])
      }

      sigs.order <- sample(1:N,N,replace=FALSE)
      for (n in sigs.order) {
        A <- rd_sample_A(n,A,M,W,P,E,p,S,N,fac,Ar,Pr,Er,params[l])
      }
    }
    ...
    if (params[l]==(-1)) {
      ...
      for (s in 1:S) {
        ...
        for (j in 1:G[s]) {
          ...
          if (length(nonzero)>0) {
            z <- rd_sample_zjs(j,s,M[[s]][nonzero,,drop=F],P.comb[nonzero,nonzero.sigs,drop=F],
    ...
    # Sample $\beta_{in}^p$
    beta_p <- rd_sample_betap(P,alpha_p,a_p,b_p,gamma)

    # Sample $\beta_{njs}^e$
    for (s in 1:S) {
      beta_e[[s]] <- rd_sample_betase(s,E[[s]],alpha_e,a_e[s],b_e[s],gamma)
    }

    # Sample beta^e_r
    for (s in 1:S) {
      beta_er[[s]] <- rd_sample_betase(s,Er[[s]],alpha_er,a_er[s],b_er[s],gamma)
    }

    # Sample $\alpha_{in}^p$
    for (i in 1:K) {
      for (n in 1:N) {
        alpha_p[i,n] <- rd_sample_alphainp(i,n,P,alpha_p,beta_p,lambda_p,gamma)
      }
    }
    ...
    for (s in 1:S) {
      for (n in 1:N) {
	      alpha_e[[s]][n,] <- rd_sample_alphanse(n,s,E[[s]],alpha_e,beta_e,lambda_e[s],gamma)
      }
    }
    ...
    for (s in 1:S) {
      for (n in 1:Nr) {
          alpha_er[[s]][n,] <- rd_sample_alphanse(n,s,Er[[s]],alpha_er,beta_er,lambda_er[s],gamma)
      }
    }
  ...
  if (first) {
    ...
    liks <- sapply(1001:5000,function(x) sum(sapply(1:S,function(s)
      rd_loglik(s,M[[s]],W[[s]],Pr_chain[[x]],Er_chain[[x]][[s]],
    ...
    return(list(BIC=bic,A=list(discovery=A,recovery=Ar),
                ...
                E=list(discovery=Eest,recovery=Erest),W=W))
  }
}

#' Run the Recovery-Discovery Sampler
#'
#' This runs the recovery-discovery sampler for multi-study NMF using COSMIC v3.2 priors.
#' Importantly, the order of mutational categories of the input data has to match that of the
#' COSMIC reference, see data('cosmic').
#'
...
#'  }
#' @export
recovery_discovery_fit <- function(M,lambda_p=0.5,a_p=5,b_p=0.05,lambda_e=0.1,a_e=5,b_e=0.01,
                                   a_er=3,b_er=0.01,lambda_er=0.5) {

  message("Make sure the order of mutational categories in the input data matches
          the COSMIC reference, see data('cosmic')")

  initial <- recovery_discovery(M,lambda_p,a_p,b_p,lambda_e,a_e,b_e,
                                a_er,b_er,lambda_er,inits=NULL,fixed=F,first=T)
  outs <- lapply(1:length(initial[[1]]),function(x)
    recovery_discovery(M,lambda_p,a_p,b_p,lambda_e,a_e,b_e,
                       a_er,b_er,lambda_er,
