...
### Full Sampler
recovery_discovery <- function(M,lambda_p=0.5,a_p=5,b_p=0.05,lambda_e=1,a_e=5,b_e=0.01,a_er=5,b_er=0.01,
  ...
  Er <- lapply(1:S,function(s) array(rgamma(Nr*G[s],alpha_e[[s]],beta_e[[s]]),dim=c(Nr,G[s])))
  ...
  for (l in 1:totaliters) {
    ...
    for (s in 1:S) {
      for (n in 1:Nr) {
        for (j in 1:G[s]) {
          beta_er[[s]][n,j] <- sample_betanjse(n,j,s,Er[[s]],alpha_er,a_er[s],b_er[s],gamma)
        }
      }
    }
  ...
  if (first) {
    ...
    return(list(marg.lik,list(A,Ar),list(Pest,Prest)))
  }  
}
